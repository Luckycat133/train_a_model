"""Reinforcement Learning trainer for GRPO/DPO using Accelerate."""
...
class PreferenceDataset(Dataset):
    def __init__(self, data_path: str, tokenizer: Any, max_length: int = 1024, max_prompt_length: int = 512):
    ...
    def _parse_item(self, item: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        chosen = item.get("chosen") or item.get("chosen_response") or item.get("output")
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        prompt_ids = self.tokenizer.encode(sample["prompt"], max_length=self.max_prompt_length, truncation=True)
        chosen_ids = self.tokenizer.encode(sample["chosen"], max_length=self.max_length - len(prompt_ids), truncation=True)
        rejected_ids = self.tokenizer.encode(sample["rejected"], max_length=self.max_length - len(prompt_ids), truncation=True)
...
class DPOTrainer(RLTrainer):
    ...
    def build_model(self) -> nn.Module:
        ...
        model = SimpleTransformer(
            ...
            dropout=cfg.dropout, max_len=cfg.max_len, mode=cfg.mode,
        )
    ...
    def _build_ref_model(self) -> nn.Module:
        ...
        ref_model = SimpleTransformer(
            vocab_size=self.config.vocab_size, d_model=self.config.d_model,
            nhead=self.config.nhead, num_layers=self.config.num_layers,
            dim_feedforward=self.config.dim_feedforward, dropout=self.config.dropout,
            max_len=self.config.max_len, mode=self.config.mode,
        )
    ...
    def compute_loss(self, batch: Dict[str, torch.Tensor], **kwargs) -> Tuple[torch.Tensor, Dict[str, float]]:
        ...
        return loss, {
            "loss": loss.item(), "chosen_acc": chosen_acc,
    ...
    def _get_extra_checkpoint_state(self) -> Dict[str, Any]:
        return {"trainer_type": "dpo", "method": "dpo"}
...
class GRPOTrainer(RLTrainer):
    ...
    def _sample_responses(self, prompts: List[str], temperature: float = 0.7,
                          top_p: float = 0.9) -> Tuple[List[List[int]], List[float]]:
        ...
        for prompt in prompts:
            ...
            group_responses, group_rewards = [], []

            for _ in range(self.config.grpo_group_size):
                response_ids = self._generate_response(prompt_tensor, temperature=temperature, top_p=top_p)
    ...
    def _generate_response(self, prompt_tensor: torch.Tensor, temperature: float = 0.7,
    ...
    def compute_loss(self, batch: Dict[str, Any], **kwargs) -> Tuple[torch.Tensor, Dict[str, float]]:
        ...
        for prompt, group_responses, group_advantages in zip(prompts, responses, advantages):
            ...
            for response_ids, advantage in zip(group_responses, group_advantages):
                ...
                response_log_probs = log_probs.gather(
                    dim=-1, index=response_tensor[:, 1:].unsqueeze(-1)).squeeze(-1)
                ...
                if self.config.kl_coef > 0:
                    ...
                    kl_div = F.kl_div(log_probs, ref_log_probs, reduction="batchmean", log_target=True)
        ...
        return total_loss, {"loss": total_loss.item(), "reward": avg_reward, "num_samples": num_samples}
    ...
    def _get_extra_checkpoint_state(self) -> Dict[str, Any]:
        return {"trainer_type": "grpo", "method": "grpo"}
...
def create_rl_trainer(
    ...
    **kwargs
    ...
    if method.lower() == "dpo":
        trainer = DPOTrainer(config=config, model=model, accelerator=accelerator)
    elif method.lower() == "grpo":
        trainer = GRPOTrainer(config=config, model=model, accelerator=accelerator)
    else:
